...
def main() -> None:
    ...
    warnings = []
    logger.info("RNA_MassHunter_v2 MVP-1 started")
    ...
    sequence = config.sequence.get("sequence", "")
    if sequence:
        theoretical_mass = calculate_unmodified_rna_mass(sequence, base_masses, warnings=warnings)
    else:
        add_warning(warnings, "WARNING", "masses", "config.sequence.sequence is empty; theoretical mass was not calculated.")
    ...
    logger.info("RNA_MassHunter_v2 MVP-1 finished")
